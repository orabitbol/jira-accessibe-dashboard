...
// Same pattern as localJiraApi, for the read-only eazyBI report proxy.
function localEazyBiApi(env) {
  return {
    name: "local-eazybi-api",
    configureServer(server) {
      server.middlewares.use("/api/eazybi", (req, res) => {
        if (req.method !== "POST") {
          res.statusCode = 405;
          res.end(JSON.stringify({ error: "Method not allowed" }));
          return;
        ...
        req.on("end", async () => {
          let parsed = {};
          try { parsed = JSON.parse(body || "{}"); } catch { /* ignore */ }
          const r = await fetchEazyBI({ reportKey: parsed.reportKey, env });
          res.statusCode = r.status;
          res.setHeader("Content-Type", "application/json");
          res.end(JSON.stringify(r.body));
        });
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), localJiraApi(env), localEazyBiApi(env)],
  };
});
